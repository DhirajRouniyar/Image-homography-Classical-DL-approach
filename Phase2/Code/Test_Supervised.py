...
#print(device)
model = HNet()
checkpoint = torch.load('./Checkpoints_Sup/7_model.ckpt') #Load the Supervised trained model
...
def predict_homographies(images):
    ...
    np.save("H4pt_pred_Sup.npy",homography)
    return homography

def main():
    
    #Get an image for Testing
    image = cv2.imread(f'../../Data/Train/10.jpg')  #Read image manually to get the results
    h, w, c = image.shape
...
##Predicting Homography
    ...
    np.save("H4pt_pred_Sup.npy",homography)
    H4pt_pred = np.load("H4pt_pred_Sup.npy").astype(np.float32)
#print(homography)
